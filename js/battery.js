...
//STEP 1d: grab the <dl> element to append img
const batteryParent = document.querySelector('#battery');
//STEP 1e: create the image element
var image;
...
function updateBatteryStatus(battery) {
    ...
    //STEP 3d: Create a dynamic url variable for the third party API
    const url = `https://robohash.org/${battery.level * 100}percent.png`

    if(!image){
        //STEP 3e: Create an image element to display battery percentage
        image = document.createElement("img");

        //STEP 3f: append the image to the dl tag
        batteryParent.appendChild(image);
    }

    //STEP 3g: Update the image with src attribute
